...
class SubdivisionInline(admin.TabularInline):
    ...
    def get_fields(self, request, obj=None):
        return ('id_display', 'name', 'division', 'staff_planned_total',
                'staff_planned_management', 'staff_planned_officers',
                'staff_planned_warrant_officers', 'staff_planned_civilian')

    def get_readonly_fields(self, request, obj=None):
        return ('id_display', 'created_at', 'updated_at')
...
class DivisionAdmin(admin.ModelAdmin):
    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        ...
        if db_field.name == 'head':
            qs = Employee.objects.filter(position='Начальник отдела')
            if obj and obj.pk:
                qs = qs.filter(division=obj)
            kwargs['queryset'] = qs
        elif db_field.name == 'deputy_head':
            qs = Employee.objects.filter(position='Заместитель начальника отдела')
            if obj and obj.pk:
                qs = qs.filter(division=obj)
            kwargs['queryset'] = qs
...
class SubdivisionAdmin(admin.ModelAdmin):
    list_display = ('id', 'name', 'division', 'head', 'deputy_head', 'staff_planned_total', 'created_at')
    search_fields = ('name',)
    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        obj = kwargs.get('obj')
        if db_field.name == 'head':
            qs = Employee.objects.filter(position='Начальник отделения')
            if obj and obj.pk:
                qs = qs.filter(subdivision=obj)
            kwargs['queryset'] = qs
        elif db_field.name == 'deputy_head':
            qs = Employee.objects.filter(position='Заместитель начальника отделения')
            if obj and obj.pk:
                qs = qs.filter(subdivision=obj)
            kwargs['queryset'] = qs
        return super().formfield_for_foreignkey(db_field, request, **kwargs)